{flake-utils-plus}: {
  ...
  configurationBuilder = reverseDomainName: host': (
    let
      ...
      in
        if domainLabels == []
        then (lib.mkDefault null) # null is the networking.domain option's default
      ...
      */
      ...
      nixosSpecialArgs = let
        f = channelName: {"${channelName}ModulesPath" = toString (channels.${channelName}.input + "/nixos/modules");};
      in
                  ...
                  (
                    ...
                    then {
                      nixpkgs.pkgs =
                        ...
                        if (hostConfig.nixpkgs.config == {})
                        then selectedNixpkgs
                        else
  ...
  mergeAny otherArguments (
    ...
    (
      system: let
        ...
        channelsFromFlakes = mapAttrs (_name: input: {inherit input;}) channelFlakes;

        importChannel = name: value:
          (import (patchChannel system value.input (value.patches or [])) {
            ...
            overlays =
              ...
              ++ sharedOverlays
              ++ (
                if (value ? overlaysBuilder)
                then (value.overlaysBuilder pkgs)
                else []
              )
